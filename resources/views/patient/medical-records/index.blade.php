    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-800 rounded-lg shadow-lg p-6 mb-6 text-white">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div><span class="text-blue-200">العمر:</span> {{ auth()->user()->patient->age ?? '-' }} سنة</div>
                <div><span class="text-blue-200">الجنس:</span> {{ auth()->user()->patient->gender == 'male' ? 'ذكر' : 'أنثى' }}</div>
                <div><span class="text-blue-200">الهاتف:</span> {{ auth()->user()->patient->phone ?? '-' }}</div>
                <div><span class="text-blue-200">فصيلة الدم:</span> {{ auth()->user()->patient->blood_type ?? 'غير محددة' }}</div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="p-6">
                <h2 class="text-xl font-bold text-gray-800 mb-4">سجل زياراتك</h2>
                <div class="space-y-4">
